...
const schema = z.object({ track_id: z.string().min(1) });
...
export async function GET(request: Request) {
    ...
    try {
        ...
        const res = await fetch(`${BACKEND_URL}/v2/tracks/${track_id}`, { headers });
        const data = await res.json();
        return new Response(JSON.stringify(data), { status: res.status });
    } catch (error: any) {
        return new Response(
            JSON.stringify({ success: false, error: error?.message || 'Track fetch error' }),
            { status: 400 }
        );
    }
}
