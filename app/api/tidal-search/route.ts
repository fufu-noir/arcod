...
import { searchTidal } from '@/lib/tidal-dl-server';
import z from 'zod';
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const searchResults = await searchTidal(q, 10, offset, country || undefined);
        return new NextResponse(JSON.stringify({ success: true, data: searchResults }), { status: 200 });
    } catch (error: any) {
        return new NextResponse(
            JSON.stringify({
                success: false,
                error: error?.errors || error.message || 'An error occurred searching Tidal.'
            }),
            { status: 400 }
        );
    }
