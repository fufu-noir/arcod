...
import PlaylistCard, { QobuzPlaylist } from '@/components/playlist-card';
...
const SearchView = () => {
    ...
    const [playlist, setPlaylist] = useState<QobuzPlaylist | null>(null);
    ...
    const parseQobuzUrl = (input: string): { type: 'album' | 'track' | 'artist' | 'playlist'; id: string } | null => {
        const trimmed = input.trim();
        if (!/^https?:\/\//i.test(trimmed)) return null;
        try {
            const u = new URL(trimmed);
            if (!/qobuz\.com|play\.qobuz\.com|open\.qobuz\.com/i.test(u.hostname)) return null;
            // Patterns: /album/:slug/:id, /track/:slug/:id, /interpreter/:slug/:id (artist), /playlists/:id
            const segments = u.pathname.split('/').filter(Boolean);
            const typeMap: Record<string, 'album' | 'track' | 'artist' | 'playlist'> = {
                album: 'album',
                track: 'track',
                interpreter: 'artist',
                artist: 'artist',
            ...
            for (let i = 0; i < segments.length; i++) {
                const type = typeMap[segments[i].toLowerCase()];
                if (!type) continue;
                // The ID is usually the last segment, which is numeric (or alphanumeric for playlists)
                for (let j = segments.length - 1; j > i; j--) {
                    const candidate = segments[j];
                    if (/^[a-z0-9_-]{3,}$/i.test(candidate)) {
    ...
    const handleUrlSearch = useCallback(async (parsed: { type: 'album' | 'track' | 'artist' | 'playlist'; id: string }) => {
        setPlaylist(null);
        try {
            if (parsed.type === 'album') {
                const res = await axios.get(`/api/get-album?album_id=${parsed.id}`, { headers: { 'Token-Country': country } });
                const album = res.data?.data;
                if (!album) throw new Error('Album not found');
                setResults({
                    albums: { total: 1, offset: 0, limit: 1, items: [album] },
                    tracks: { total: 0, offset: 0, limit: 0, items: [] },
                    artists: { total: 0, offset: 0, limit: 0, items: [] },
                } as any);
                return;
            }
            if (parsed.type === 'artist') {
                const res = await axios.get(`/api/get-artist?artist_id=${parsed.id}`, { headers: { 'Token-Country': country } });
                const d = res.data?.data?.artist || res.data?.data;
                if (!d) throw new Error('Artist not found');
                // name can be { display: "..." } or a plain string
                const artistName = typeof d.name === 'object' ? (d.name?.display || d.name?.name || '') : (d.name || '');
                const portraitHash = d.images?.portrait?.hash;
                const portraitFmt = d.images?.portrait?.format || 'jpg';
                const builtUrl = portraitHash
                    ? `https://static.qobuz.com/images/artists/covers/medium/${portraitHash}.${portraitFmt}`
                    : null;
                const img = d.image?.large || d.image?.medium || d.image?.small || builtUrl;
                let albums_count = d.albums_count;
                if (albums_count == null) {
                    try {
                        const types = ['album', 'epSingle', 'live', 'compilation'];
                        const counts = await Promise.all(types.map(t =>
                            axios.get(`/api/get-releases?artist_id=${parsed.id}&release_type=${t}&limit=1&offset=0`, { headers: { 'Token-Country': country } })
                                .then(r => r.data?.data?.has_more ? 99 : (r.data?.data?.items?.length ?? 0))
                                .catch(() => 0)
                        ));
                        albums_count = counts.reduce((a, b) => a + b, 0) || null;
                    } catch { /* ignore */ }
                }
                const artist = {
                    id: d.id,
                    ...
                    albums_count,
                    picture: img,
                    image: d.image || (img ? { small: img, medium: img, large: img, extralarge: img, mega: img } : null),
                };
                setResults({
                    albums: { total: 0, offset: 0, limit: 0, items: [] },
                    tracks: { total: 0, offset: 0, limit: 0, items: [] },
                    artists: { total: 1, offset: 0, limit: 1, items: [artist] },
                } as any);
                return;
            }
            if (parsed.type === 'track') {
                const res = await axios.get(`/api/get-track?track_id=${parsed.id}`, { headers: { 'Token-Country': country } });
                const track = res.data?.data;
                if (!track) throw new Error('Track not found');
                setResults({
                    albums: { total: 0, offset: 0, limit: 0, items: [] },
                    tracks: { total: 1, offset: 0, limit: 1, items: [track] },
                    artists: { total: 0, offset: 0, limit: 0, items: [] },
                } as any);
                return;
            }
            if (parsed.type === 'playlist') {
                const res = await axios.get(`/api/get-playlist?playlist_id=${parsed.id}`, { headers: { 'Token-Country': country } });
                const pl = res.data?.data as QobuzPlaylist;
                if (!pl) throw new Error('Playlist not found');
                setPlaylist(pl);
                setResults(null);
                setQuery(pl.name || `Playlist ${parsed.id}`);
                return;
            }
        } catch (error: any) {
            setSearchError(error?.response?.data?.error || error.message || 'Could not load the link.');
        }
    }, [country]);
    ...
    const onSearch = useCallback(async (query: string) => {
        ...
        if (parsedUrl) {
            await handleUrlSearch(parsedUrl);
            setSearching(false);
            setLoading(false);
            return;
        }
        ...
        setSearching(false);
    }, [country, getSearchEndpoint, handleUrlSearch]);
                ...
                <div className='my-8 w-full mx-auto max-w-[1600px] pb-20'>
                    <div className='mb-10 animate-in fade-in slide-in-from-bottom-4 duration-300'>
                        <div className='flex items-center justify-between mb-4 px-4 md:px-6'>
                            <div className='flex items-center gap-3'>
                                <div className='p-2 rounded-lg bg-primary/10'>
                                    <DiscAlbumIcon className='w-5 h-5 text-primary' />
                                </div>
                        ...
                        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6 w-full px-4 md:px-6 overflow-visible'>
                            <div className='animate-in fade-in zoom-in-95 duration-200'>
                                <PlaylistCard playlist={playlist} resolvedTheme={String(resolvedTheme)} />
                            </div>
                        </div>
                    </div>
                </div>
