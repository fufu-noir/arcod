...
import { fetchLyrics } from './lyrics-fetcher';
        ...
        await createJob(setStatusBar, formattedTitle, Disc3Icon, async () => {
            return new Promise(async (resolve) => {
                try {
                    ...
                    if (settings.embedLyrics && settings.applyMetadata) {
                        setStatusBar((prev) => ({ ...prev, description: 'Fetching lyrics...' }));
                        lyrics = await fetchLyrics(result as QobuzTrack);
                    }
                    let outputFile = await applyMetadata(inputFile, result as QobuzTrack, ffmpegState, settings, setStatusBar, undefined, undefined, lyrics);
                    ...
                    // Guard: always resolve within 10s even if audio element events never fire
                    ...
                    const metadataTimeout = setTimeout(() => {
                        // Event never fired — proceed with download anyway to unblock the queue
                    ...
                    audioElement.onerror = function () {
                        // Audio element failed to load — still download and unblock queue
                        clearTimeout(metadataTimeout);
                        proceedDownload(objectURL, title);
                        resolveOnce();
                    };
        ...
        await createJob(setStatusBar, formattedZipTitle, DiscAlbumIcon, async () => {
            return new Promise(async (resolve) => {
                try {
                    ...
                    if (!fetchedAlbumData?.tracks?.items) {
                        toast({ title: 'Error', description: 'Could not load album tracks', variant: 'destructive' });
                        resolve();
                        return;
                    }
                    ...
                    for (const [index, url] of albumUrls.entries()) {
                        if (url) {
                            ...
                            let lyrics: string | null = null;
                            if (settings.embedLyrics && settings.applyMetadata) {
                                lyrics = await fetchLyrics(albumTracks[index]);
                            }
...
export async function downloadArtistDiscography(
    ...
    for (const type of types) {
        ...
        const items = artistResults?.artist?.releases?.[type]?.items || [];
