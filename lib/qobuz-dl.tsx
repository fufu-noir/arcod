...
export function formatDuration(seconds: number) {
    ...
    return `${hours > 0 ? hours + 'h ' : ''} ${hours > 0 || remainingMinutes > 0 ? remainingMinutes + 'm ' : ''} ${remainingSeconds < 10 ? '0' : ''}${remainingSeconds}s`.trim();
}
...
export function parseArtistData(artistData: QobuzArtistResults) {
    ...
    // Early return if no releases data
    if (!artistData?.artist?.releases) return artistData;

    const releases = artistData.artist.releases as any;
    if (!Array.isArray(releases) || releases.length === 0) return artistData;
    ...
    for (const type of ['album', 'live', 'compilation', 'epSingle']) {
        const foundRelease = releases.find((release: any) => release.type === type);
        if (!foundRelease) continue;
        newReleases[type] = {
            has_more: foundRelease?.has_more || false,
...
export async function getFullAlbumInfo(
    ...
    const albumDataResponse = await axios.get('/api/get-album', { params: { album_id: (result as QobuzAlbum).id, title: result.title, artist: result.artist?.name }, headers: { 'Token-Country': country } });
