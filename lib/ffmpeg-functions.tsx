...
export async function applyMetadata(
    ...
    const isMP3 = settings.outputCodec === 'MP3';
    ...
    if (lyrics) {
        ...
        // In FFMETADATA, special characters = ; # \\ and newline need escaping
    ...
    const metadataArgs = ['-i', 'input.' + extension, '-i', 'metadata.txt', '-map_metadata', '1', '-codec', 'copy'];
    if (isMP3) metadataArgs.push('-id3v2_version', '3');
    metadataArgs.push('secondInput.' + extension);
    await ffmpeg.run(...metadataArgs);
    ...
    if (isMP3) coverArgs.push('-id3v2_version', '3');
    coverArgs.push('output.' + extension);
    await ffmpeg.run(...coverArgs);
...
export function createFFmpeg() {
    ...
    if (!isSharedArrayBufferAvailable()) {
        console.warn('[FFmpeg] SharedArrayBuffer not available - FFmpeg disabled. Downloads will work but without re-encoding/metadata.');
        return null;
    }
