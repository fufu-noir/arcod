...
export async function search(query: string, limit: number = 10, offset: number = 0, options?: APIOptionProps) {
    ...
    url.searchParams.append('app_id', process.env.QOBUZ_APP_ID!);
...
export async function getArtist(artistId: string, options?: APIOptionProps): Promise<QobuzArtist | null> {
    ...
    return (
        await axios.get(process.env.CORS_PROXY ? process.env.CORS_PROXY + encodeURIComponent(url.toString()) : url.toString(), {
            params: { artist_id: artistId, sort: 'release_date', app_id: process.env.QOBUZ_APP_ID! },
            headers: {
                'x-user-auth-token': token,
                'User-Agent': process.env.CORS_PROXY ? 'Qobuz-DL' : undefined
            },
...
export async function getAlbumInfo(album_id: string, options?: APIOptionProps, meta?: { albumTitle?: string; artistName?: string }) {
    testForRequirements();
    const { country, ...requestOptions } = options || {};
    const token = country ? getTokenForCountry(country) : getRandomToken();
    ...
    const fetchAlbum = async (id: string, useToken = true) => {
        ...
        if (useToken) headers['x-user-auth-token'] = token;
        const response = await axios.get(process.env.CORS_PROXY ? process.env.CORS_PROXY + encodeURIComponent(url.toString()) : url.toString(), {
            headers,
            httpAgent: proxyAgent,
            httpsAgent: proxyAgent,
    ...
    // Try with token first, then without token (region fallback), then search fallback
    try {
        return await fetchAlbum(album_id, true);
    } catch {
        // Token might restrict by region — retry without token
        try {
            return await fetchAlbum(album_id, false);
        } catch {
            // Both failed — try search fallback if we have metadata
        }
    }

    if (meta?.albumTitle && meta?.artistName) {
        console.log(`[Qobuz] album/get failed for ${album_id}, searching: "${meta.artistName} ${meta.albumTitle}"`);
        try {
            const searchUrl = new URL(process.env.QOBUZ_API_BASE + 'catalog/search');
            searchUrl.searchParams.append('query', `${meta.artistName} ${meta.albumTitle}`);
            searchUrl.searchParams.append('type', 'albums');
            searchUrl.searchParams.append('limit', '10');
            searchUrl.searchParams.append('app_id', process.env.QOBUZ_APP_ID!);
            const searchFullUrl = process.env.CORS_PROXY ? process.env.CORS_PROXY + encodeURIComponent(searchUrl.toString()) : searchUrl.toString();
            const searchResponse = await axios.get(searchFullUrl, {
                headers: {
                    'x-app-id': process.env.QOBUZ_APP_ID!,
                    'x-user-auth-token': token,
                    'User-Agent': process.env.CORS_PROXY ? 'Qobuz-DL' : undefined
            ...
            if (Array.isArray(albums) && albums.length > 0) {
                const titleLower = meta.albumTitle.toLowerCase().trim();
                const artistLower = meta.artistName.toLowerCase().trim();
                const match = albums.find((a: any) =>
                    a.title?.toLowerCase().trim() === titleLower &&
                    a.artist?.name?.toLowerCase().trim() === artistLower
                ) || albums[0];
                console.log(`[Qobuz] Search fallback: "${meta.albumTitle}" → ${match.id}`);
                // Use without token since the original ID was region-restricted
                return await fetchAlbum(String(match.id), false);
            }
        } catch (searchErr: any) {
    ...
    throw new Error(`Album ${album_id} not found (region-restricted or unavailable)`);
}
...
export async function getDownloadURL(trackID: number, quality: string, options?: APIOptionProps) {
    ...
    try {
        ...
        console.error('Qobuz API Error Details:', e.response?.data || e.message);
        throw e;
    }
}
