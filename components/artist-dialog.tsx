...
const ArtistDialog = ({ open, setOpen, artist }: { open: boolean; setOpen: (open: boolean) => void; artist: QobuzArtist }) => {
    ...
    const getArtistData = useCallback(async () => {
        ...
        try {
            ...
            const parsed = parseArtistData(response.data.data);
            // /artist/page returns a curated limited set — reset all categories to empty
            // so getReleasesList loads the full catalog via lazy loading
            if (parsed?.artist) {
                parsed.artist.releases = {
                    album:       { items: [], has_more: true },
                    epSingle:    { items: [], has_more: true },
                    live:        { items: [], has_more: true },
                    compilation: { items: [], has_more: true },
                };
            }
            setArtistResults(parsed);
        } catch {
    ...
    const fetchMore = useCallback(async (searchField: 'album' | 'epSingle' | 'live' | 'compilation', artistResults: QobuzArtistResults) => {
        ...
        try {
            ...
            const newItems = (response.data.data?.items || []).map((release: any) => {
                try { return parseArtistAlbumData(release); } catch { return release; }
            });
            const has_more = newItems.length > 0 && (response.data.data?.has_more ?? newItems.length >= 20);
            setArtistResults({
                ...artistResults!,
                artist: {
            ...
            // On error, stop loading more to prevent infinite retry loop
            setArtistResults({
                ...artistResults!,
                artist: {
                    ...artistResults!.artist,
    ...
    const qobuzImage = artist.image?.large || artist.image?.small ||
    ...
    const [deezerArtistImage, setDeezerArtistImage] = useState<string | null>(null);
    useEffect(() => {
        if (!open || !artist?.name) return;
        let cancelled = false;
        const url = `${process.env.NEXT_PUBLIC_AWS_API_URL}/artist-image?name=${encodeURIComponent(artist.name)}`;
        fetch(url)
            .then(r => r.ok ? r.json() : null)
            .then(data => { if (!cancelled && data) setDeezerArtistImage(data.imageUrl || null); })
            .catch(() => { /* ignore */ });
        return () => { cancelled = true; };
    }, [open, artist?.name]);

    const artistImage = deezerArtistImage || qobuzImage;
                            ...
                            >
                                ...
                                <div className='flex gap-2 overflow-x-auto flex-1'>
                                    {artistReleaseCategories.map((category) => {
                                        const rel = artistResults?.artist?.releases?.[category.value];
                                        // Hide tab only when confirmed empty: loaded at least once and no items
                                        const confirmedEmpty = artistResults && rel && !rel.has_more && rel.items.length === 0;
...
const ArtistReleaseGrid = memo(({
    ...
    // Initial load when items are empty but has_more (fresh category)
    // Also load more when searching and there's more data
    useEffect(() => {
        const isEmpty = allReleases.length === 0 && hasMore && !searchQuery.trim();
        const isSearchFetch = searchQuery.trim() && hasMore;
        if ((isEmpty || isSearchFetch) && !loadingRef.current) {
            loadingRef.current = true;
            setLoading(true);
            fetchMore(category.value, artistResults).finally(() => {
                loadingRef.current = false;
                setLoading(false);
            });
        }
    }, [searchQuery, hasMore, allReleases.length, fetchMore, category.value, artistResults]);

    // Intersection observer for loading more from API
    useEffect(() => {
        const observer = new IntersectionObserver(
            (entries) => {
                if (entries[0].isIntersecting && !searchQuery.trim() && hasMore && !loadingRef.current) {
                    loadingRef.current = true;
                    setLoading(true);
                    fetchMore(category.value, artistResults).finally(() => {
                        loadingRef.current = false;
    ...
    }, [hasMore, searchQuery, fetchMore, category.value, artistResults]);

    const isSearching = searchQuery.trim() && hasMore;

    if (filteredReleases.length === 0 && !isSearching && !hasMore && !loading) {
        return (
            <div className={cn(
