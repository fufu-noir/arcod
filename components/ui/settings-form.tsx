...
import { ChevronDownIcon, DotIcon, InfoIcon, LockIcon, SettingsIcon } from 'lucide-react';
    ...
    return (
        <Sheet open={open} onOpenChange={setOpen} modal={true}>
            <Button
                variant='ghost'
                size='sm'
                className='h-8 w-8 p-0 text-muted-foreground hover:text-foreground transition-colors rounded-full'
                onClick={() => {
                    setOpen(true);
                }}
            >
                ...
                <div className='flex flex-col gap-4 pt-4 h-full overflow-y-scroll scrollbar-hide'>
                    <SheetHeader>
                        <div className='flex flex-col my-1'>
                            ...
                            <SheetDescription>Change the way Arcod looks</SheetDescription>
                        </div>
                        <ModeToggle />
                    </SheetHeader>
                    <Separator />
                    <SheetHeader>
                        <div className='flex flex-col my-1'>
                            ...
                            <SheetDescription>Change the background of Arcod</SheetDescription>
                        </div>
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant='outline' className='flex gap-2 items-center'>
                                    <p className='capitalize'>{settings.background === 'particles' ? 'Particles' : settings.background === 'futuristic' ? 'Futuristic' : 'Solid Color'}</p>
                                    <ChevronDownIcon />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align='start'>
                                <DropdownMenuRadioGroup
                                    ...
                                    onValueChange={(value: string) => {
                                        setSettings((prev) => ({ ...prev, background: value as 'particles' | 'solid' | 'futuristic' }));
                                    }}
                                >
                                    ...
                                    <DropdownMenuRadioItem value='futuristic'>Futuristic</DropdownMenuRadioItem>
                                </DropdownMenuRadioGroup>
                            </DropdownMenuContent>
                        </DropdownMenu>
                    </SheetHeader>
                    ...
                    <SheetHeader className='space-y-3'>
                        <div className='flex items-center gap-2'>
                            <div className='flex flex-col'>
                                <p className={cn('font-medium', settings.outputCodec === 'WAV' && 'text-muted-foreground')}>Download Lyrics</p>
                                <p className={cn('text-xs', settings.outputCodec === 'WAV' ? 'text-muted-background' : 'text-muted-foreground')}>
                                    Download synced lyrics from Musixmatch with your music files.<br></br>
                                    <em>(If no lyrics are found, it will use the lyrics from Apple Music or Spotify.)</em>
                                </p>
                            </div>
                            <Checkbox
                                checked={settings.embedLyrics && settings.outputCodec !== 'WAV'}
                                onCheckedChange={(checked: boolean) => setSettings((settings) => ({ ...settings, embedLyrics: checked }))}
                                disabled={settings.outputCodec === 'WAV'}
                            />
                        </div>
                        {settings.embedLyrics && settings.outputCodec !== 'WAV' && (
                            <div className='space-y-2 pl-1'>
                                <p className='font-medium text-sm'>Lyrics Mode</p>
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button variant='outline' className='flex gap-2 items-center w-full justify-between'>
                                            <p>{settings.lyricsMode === 'embedded' ? 'Embedded in file' : 'Separate .lrc file'}</p>
                                            <ChevronDownIcon />
                                        </Button>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent align='start' className='w-[280px]'>
                                        <DropdownMenuRadioGroup
                                            ...
                                            onValueChange={(mode: string) => {
                                                setSettings((prev) => ({ ...prev, lyricsMode: mode as 'embedded' | 'sidecar' }));
                                            }}
                                        >
                                            <DropdownMenuRadioItem value='embedded' className='flex-col items-start gap-1'>
                                                <span className='font-medium'>Embedded in file</span>
                                                <span className='text-xs text-muted-foreground'>Lyrics are saved inside the audio file metadata</span>
                                            </DropdownMenuRadioItem>
                                            <DropdownMenuRadioItem value='sidecar' className='flex-col items-start gap-1'>
                                                <span className='font-medium'>Separate .lrc file</span>
                                                <span className='text-xs text-muted-foreground'>Lyrics are downloaded as separate .lrc files alongside music</span>
                                            </DropdownMenuRadioItem>
                                        </DropdownMenuRadioGroup>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                    ...
                    <SheetHeader className='space-y-3'>
                        <div className='flex items-center gap-2'>
                            <div className='flex flex-col'>
                                <p className='font-medium'>Download Booklet</p>
                                <p className='text-xs text-muted-foreground'>
                                    If available, download the digital booklet (PDF) included with the release.
                                </p>
                            </div>
                            <Checkbox
                                checked={settings.downloadBooklet}
                                onCheckedChange={(checked: boolean) => setSettings((prev) => ({ ...prev, downloadBooklet: checked }))}
                            />
                        </div>
                    </SheetHeader>
                    <Separator />
                    <SheetHeader className='space-y-3'>
                        <div className='flex items-center gap-2'>
                            <div className='flex flex-col'>
                                <p className='font-medium'>Attach Cover Art</p>
                                <p className='text-xs text-muted-foreground'>
                                    Include a cover.jpg file alongside the tracks when downloading an album.
                                </p>
                            </div>
                            <Checkbox
                                checked={settings.attachCover}
                                onCheckedChange={(checked: boolean) => setSettings((prev) => ({ ...prev, attachCover: checked }))}
                            />
                        </div>
                    </SheetHeader>
                    <Separator />
                    <SheetHeader className='space-y-4'>
                        ...
                        <div className='space-y-2'>
                            ...
                            {isGuest ? (
                                <div className='space-y-2'>
                                    <Button variant='outline' className='flex gap-2 items-center cursor-not-allowed opacity-70' disabled>
                                        <LockIcon className='w-3.5 h-3.5' />
                                        <p>MP3 — 320 kbps</p>
                                    </Button>
                                    <p className='text-xs text-amber-500/90 font-medium text-center'>
                                        Guest accounts are limited to MP3 320kbps.<br />
                                        Create a free account to unlock FLAC, WAV, ALAC, AAC, and OPUS.
                                    </p>
                                </div>
                            ) : (
                            <DropdownMenu>
