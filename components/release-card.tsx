...
const ReleaseCard = ({
    ...
    const libraryLookupHints = resultType === 'artists'
        ? undefined
        : {
            title: 'title' in result ? result.title : '',
            artistName: ('artist' in result && result.artist?.name) || formatArtists(result as QobuzAlbum | QobuzTrack),
            type: resultType === 'tracks' ? 'track' as const : 'album' as const
        };

    const libraryItem = getLibraryItem(result.id, libraryLookupHints);
    ...
    // For artist search results, fetch Deezer image (cached server-side)
    const isArtistResult = getType(result) === 'artists';
    const [deezerArtistImage, setDeezerArtistImage] = useState<string | null>(null);
    useEffect(() => {
        if (!isArtistResult) return;
        const artistName = formatTitle(result);
        if (!artistName) return;
        let cancelled = false;
        const url = `${process.env.NEXT_PUBLIC_AWS_API_URL}/artist-image?name=${encodeURIComponent(artistName)}`;
        fetch(url)
            .then(r => r.ok ? r.json() : null)
            .then(data => { if (!cancelled && data) setDeezerArtistImage(data.imageUrl || null); })
            .catch(() => { /* ignore */ });
        return () => { cancelled = true; };
    }, [isArtistResult, result]);
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    {inLibrary && (
                        <div className='absolute top-2 right-2 z-[6] flex items-center gap-1.5 rounded-[4px] border border-green-500/35 bg-black/65 px-2.5 py-1 shadow-lg backdrop-blur-md pointer-events-none'>
                            <CheckCircle2 className='h-3.5 w-3.5 text-green-400' />
                            <span className='text-[10px] font-bold uppercase tracking-[0.14em] text-green-300'>In Library</span>
                        </div>
                    )}
    ...
    ? album.genre.name
    : (result as QobuzArtist).albums_count != null
        ? `${(result as QobuzArtist).albums_count} Releases`
        : `${(result as QobuzArtist).name}`
}
                                    </p>
                                    {!(getType(result) === 'artists') && (
                                            ...
                                            <Button
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    if (inLibrary && libraryItem) {
                                                        ...
                                                        await downloadFromLibrary(libraryItem, toast, authToken);
                                                        setIsDownloading(false);
                                                    } else {
                    ...
                    <div className={cn('absolute left-0 top-0 z-[2] w-full aspect-square')}>
                        {(() => {
                            // For artists: prefer Deezer image (cached, optimized), fallback to Qobuz
                            const isArtist = getType(result) === 'artists';
                            const artistImage = isArtist ? (result as QobuzArtist).image : null;
                            const imageSrc = isArtist
                                ? (deezerArtistImage || artistImage?.mega || artistImage?.extralarge || artistImage?.large || artistImage?.medium || artistImage?.small)
                                                    ...
                                                    <div className='w-full text-center md:text-left'>
                                                        <div className='mb-2'>
                                                            <div className='flex items-center justify-center md:justify-start gap-3 mb-1'>
                                                                {(result as QobuzAlbum | QobuzTrack).parental_warning && (
                                                                    <span className='px-1.5 py-0.5 bg-primary rounded-md text-[10px] font-black text-primary-foreground shadow-lg'>EXPLICIT</span>
                                                                )}
                                                                ...
                                                                {fetchedAlbumData && fetchedAlbumData.tracks?.items ? (
                                                                    (fetchedAlbumData?.tracks?.items || []).map((track: QobuzTrack, idx: number) => {
                                                                        ...
                                                                        const libraryItem = getLibraryItem(track.id, {
                                                                            title: track.title,
                                                                            artistName: track.performer?.name || fetchedAlbumData?.artist?.name,
                                                                            type: 'track'
                                                                        });
                                                        ...
                                                        <div className='mt-4 pt-4 border-t border-white/10'>
                                                            <div className="flex flex-col gap-2">
                                                                <div className="flex overflow-hidden h-14">
                                                                    {/* Left half — full width initially, shrinks to 50% on split */}
                                                                    <motion.div
                                                                    ...
                                                                    {/* Right half — grows from 0 to 50% on split */}
                                                                    <motion.div
                                                                        className="overflow-hidden h-full shrink-0"
